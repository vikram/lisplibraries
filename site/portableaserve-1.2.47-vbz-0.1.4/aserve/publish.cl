		...
		(dolist (hostpair handlers
			  ...
			  (if* (not remove)
			     then (push (make-host-handler :host host
							   :entity ent)
					handlers)))
		  (if* (eql host (host-handler-host hostpair))
		     then ; a match
			  (if* remove
			     then (setq handlers
  ...
  (let* ((postfix nil)
    ...
    (let ((type (acl-compat.excl::filesys-type realname)))
	 ...
	 then ; Try the indexes (index.html, index.htm, or user-defined).
	      ...
	      (if* (not (eql #\/ (schar realname (1- (length realname)))))
    ...
    (with-standard-io-syntax
      (let* ((sock (request-socket req))
	     ...
	     (sos-ef) ; string output stream external format
             #+sb-unicode
             (external-format (gray-stream:external-format sock))
	     )
	   ...
	   then ; must get data to send from the string output stream
		(setq content 
		  (if* (request-reply-stream req)
		     then #+allegro
                          (setq sos-ef (stream-external-format 
