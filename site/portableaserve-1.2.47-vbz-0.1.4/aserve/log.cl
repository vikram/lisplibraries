...
(defmethod logmess (message)
  ...
  (let ((vhost (wserver-default-vhost *wserver*)))
    (logmess-stream message
                    (vhost-error-stream vhost)
                    #-allegro :lock
                    #-allegro (vhost-stream-lock vhost))))



(defmethod logmess-stream (message stream #-allegro &key #-allegro lock)
     ...
     then (let* ((ipaddr (acl-compat.socket:remote-host (request-socket req)))
		 ...
		 (lock #+allegro (and (streamp stream)
			    (getf (excl::stream-property-list stream) 
				  :lock))
                       #-allegro (vhost-stream-lock (request-vhost req))))
