...
(defun uriencode-string (str &key (external-format 
  ...
  ;; It may return the same string if no encoding need be done
  ;;
  #-sb-unicode (declare (ignore external-format))
  (let ((len #-sb-unicode (length str))
	(count 0))
    (let (#-sb-unicode
          (byte-string (loop :with result = (make-array len :element-type '(unsigned-byte 8))
                             :for i :from 0 :below len
                             :do (setf (aref result i) (char-code (aref str i)))
                             :finally (return result)))
          #+sb-unicode
          (byte-string (string-to-octets str
                                         :external-format external-format)))
      #+sb-unicode
      (setf len (length byte-string))
      ;; count the number of encodings that must be done
      (dotimes (i len)
...
(defmacro with-tohex-cvt-buffer ((buffer-var str) &body body)
  ...
  #-(or sb-unicode (and allegro (version>= 6 0)))
	 ...
	 then ; great, no encoding necessary
		...
		(do ((from 0 (1+ from))
		  ...
		  (let* ((code (buf-elt buf from)))
		    (if* (= code #.(char-code #\space))
		       then ; space -> +
			    (setf (schar ret to) #\+)
			    (incf to)
		     elseif (= code #.(char-code #\linefeed))
  ...
  (let ((res nil)
	...
	(if* (or (eql ch #\=)
	   ...
	   then (setq obj (buffer-substr str start i))
		(setq start (1+ i))
	 elseif (= i max-minus-1)
	   then (setq obj (buffer-substr str start (1+ i)))
	 elseif (and (not seenpct) (or (eql ch #\%)
				       (eql ch #\+)))
	   then (setq seenpct t)
...
#+sb-unicode
(declaim (inline hex-digit-to-number))
(defun hex-digit-to-number (ch)
...
#+sb-unicode
(defun un-hex-escape (str spacep &key (external-format
                                       *default-aserve-external-format*))
  (unless (find-if #'(lambda (c)
                       (or (eql c #\%)
                           (and spacep (eql c #\+))))
                   str)
    (return-from un-hex-escape str))
  (let* ((length (length str))
    ...
    (do ((i 0 (1+ i))
         ...
         (state :normal)
         (first-digit 0)
         (previous-escape-sequence 0))
        ((>= i length)
         (octets-to-string octets :end count :external-format external-format))
      (setf ch (aref str i))
      (case state
        (:normal
         (cond ((eql ch #\%)
                (setf state :hex-digit-1))
               ((and spacep (eql ch #\+))
                (setf (aref octets count) #.(char-code #\Space))
                (incf count))
               (t
                (setf (aref octets count) (char-code ch))
                (incf count))))
        (:hex-digit-1
         (setf first-digit (hex-digit-to-number ch))
         (setf state :hex-digit-2))
        (:hex-digit-2
         (let ((code (+ (ash first-digit 4) (hex-digit-to-number ch))))
           (when (and (= previous-escape-sequence #.(char-code #\Return))
                      (= code #.(char-code #\Newline)))
             (decf count))
           (setf (aref octets count) code)
           (incf count)
           (setf previous-escape-sequence code)
           (setf state :normal)))))))

#-(or sb-unicode allegro)
	  ...
	  (let ((ch (schar given from)))
	    (if* (eql ch #\%)
	       then (let ((newchar 
		      ...
		      (if* (and (eql newchar #\linefeed)
				(> to 0)
				(eql (buf-elt str (1- to)) #\return))
...
(defun base64-decode (string)
  ...
  (let ((res (make-array (length string) :element-type 'character
	...
	(let ((ch (char string i)))
	  (if* (or (eql ch #\linefeed) (eql ch #\return))
	     then (incf i)
		  (if* (>= i (length string)) 
