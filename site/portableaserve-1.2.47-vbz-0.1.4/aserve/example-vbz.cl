...
(defun start-server (&rest args
                     &key (port 8000) (external-format :utf-8)
                     &allow-other-keys)
  (apply #'net.aserve:start :port port :external-format external-format args))

(defun stop-server ()
  (net.aserve:shutdown))

(defun start-simple-server (&key (port 8000) (external-format :utf-8))
