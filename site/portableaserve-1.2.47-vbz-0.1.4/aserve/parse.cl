...
(defun parse-http-command (buffer end)
  ...
  (let ((blankpos)
    ...
    (let ((url (buffer-substr buffer urlstart blankpos))
	 ...
	 then (if* (eql #\0 (schar buffer (+ 8 blankpos)))
		 then (setq prot :http/1.0)
	       elseif (eql #\1 (schar buffer (+ 8 blankpos)))
  ...
  (let ((len (- (length buff) 500)) ; leave space for index at end
	...
	(case state
	  (0 ; seen nothing interesting
	   (if* (eql ch #.(char-code #\linefeed))
	      then (setq state 2)
	    elseif (eql ch #.(char-code #\return))
	      then (setq state 1)))
	  (1 ; seen cr
	   (if* (eql ch #.(char-code #\linefeed))
	      then (setq state 2)
	   ...
	   then ; all done
		...
		(if* (>= i 2)
		   then (decf i 2)  ; i points at cr if there is one
			(if* (not (eql (aref buff i) #.(char-code #\return)))
			   then (incf i)))
  ...
  (let ((start (svref (parseobj-start po) index))
    ...
    (loop
	 ...
	 then (return-from trimmed-parseobj "")
		...
		(if* (eql ch-space (svref *syntax-table*
				       (char-code ch)))
		   then (incf start)
		   else (return)))))
	...
	(if* (not (eql ch-space (svref *syntax-table* (char-code ch))))
	   then (incf end)
		(return))))
		   ...
		   then ; end this one
			...
			(if* (and count (zerop (decf count)))
			   then ; get out now
				...
				(return))
		 elseif (and magic-parens (eql ch #\())
  ...
  (let ((state 0)
    ...
    (loop
	 ...
	 then (setq ch #\space)
      ...
      (setq spacep (eql ch-space (svref *syntax-table* (char-code ch))))
  ...
  (let ((len1 (length val1))
    ...
    (if* (<= len1 len2)
       then (dotimes (i len1 t)
	      (if* (not (eql (schar val1 i) (schar val2 i)))
		 then (return nil))))))
  ...
  (let ((len1 (length val1))
    ...
    (if* (<= len1 len2)
       then (let ((diff (- len2 len1)))
	      (dotimes (i len1 t)
		(if* (not (eql (schar val1 i) (schar val2 (+ diff i))))
		   then (return nil)))))))
