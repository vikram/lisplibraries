	...
	(loop
	  ...
	  (case state
		...
		then ; end of one header's value
			 ...
			 (if* (>= beginhv back)
			    then (return)
			  elseif (eql (aref buff beginhv) #.(char-code #\space))
			    then (incf beginhv)
			    else (return)))
	...
	(header-buffer-values cache-block i)
	  ...
	  (and rstart cstart
		 ...
		 (do ((rr rstart (1+ rr))
		      (cc cstart (1+ cc)))
		     ((>= rr rend))
		   (if* (not (eql (aref request-block rr)
				  (aref cache-block cc)))
