...
(progn
  ...
  (defun unix-kill (pid signal) (sb-posix:kill pid signal)))
...
(defvar *default-aserve-external-format*
  #-(or lispworks sbcl) :latin1-base
  #+(or lispworks sbcl) :latin-1) 
...
(defclass wserver ()
  ...
  (
   ...
   (external-format
    ...
    :initform #+allegro :latin1-base #-allegro :latin-1
...
(defclass vhost ()
  ((log-stream :accessor vhost-log-stream
   ...
   (error-stream :accessor vhost-error-stream
		 ...
		 :initform (ensure-stream-lock *trace-output*))
   #-allegro
   ;; There's a common lock for both streams
   (stream-lock :accessor vhost-stream-lock
...
(defun start (&key (port 80 port-p) 
  ...
  (let* ((main-socket (acl-compat.socket:make-socket :connect :passive
    ...
    (let ((*wserver* server)) ; bind it too for privacy
      (if* (or (null listeners) (eql 0 listeners))
...
(defun connection-reset-error (c)
  ...
  ;; peer
  #-sbcl
  (and (typep c 'stream-error)
       (or #+(or allegro lispworks)
           (eq (stream-error-identifier c) :connection-reset)
           #+(and allegro unix)
           (eql (stream-error-code c) 32) ; sigpipe
           #+(and allegro aix)
           (eql (stream-error-code c) 73)
           #+openmcl
  ...
  #+sbcl
  (typecase c
    (sb-bsd-sockets:not-connected-error t)
    (sb-int:simple-stream-error
     (let ((errstr (third (simple-condition-format-arguments c))))
       (and (stringp errstr)
            (or (string= "Broken pipe" errstr)
                (string= "Connection reset by peer" errstr)))))))
...
(defun get-request-body-retrieve (req)
  ...
  (let (#+allegro
        (original-ef (stream-external-format (request-socket req)))
        #+sb-unicode
        (original-ef (gray-stream:external-format (request-socket req))))
    ...
    #+sb-unicode
    (setf (gray-stream:external-format (request-socket req)) :latin-1)
	...
	(if* (member (request-method req) '(:put :post))
      ...
      #+sb-unicode
      (setf (gray-stream:external-format (request-socket req)) original-ef)
      )))
...
(defun search-usb8 (buffer start end pattern)
  ...
  (do* ((i start (1+ i))
    ...
    (dotimes (j patlen (return-from search-usb8 i))
      (if* (not (eql (aref buffer (+ i j))  (aref pattern j)))
	 then ; no match
	      (return nil)))))
			...
			then ; matched the whole boundary
				 ...
				 ((>= jj end)
				  ...
				  thenret ; pass over
					...
					elseif (and (eql (aref mpbuffer jj)
							 #.(char-code #\-))
						    (eql (aref mpbuffer (1+ jj))
							 #.(char-code #\-)))
					  then (return-from scan-forward
			...
			then ; end of buffer before end of boundary
		     ...
		     (let ((bufch (aref mpbuffer j)))
			  ...
			  then (incf bufch #.(- (char-code #\a)
						(char-code #\A))))
		       (if* (not (eql bufch (aref boundary ind)))
			  then (return))))))))))) 
...
(defmethod get-multipart-sequence ((req http-request)
  ...
  #-(or (and allegro (version>= 6 0 pre-final 1)) sb-unicode)
  (if* ef-spec
     then (warn "~
		      ...
		      then 
			   ...
			   (multiple-value-setq (buffer items tocopy)
			     (octets-to-string
			      ...
			      :truncate t))
                           #+sb-unicode
                           (let ((s (octets-to-string mpbuffer
                                                      :start cur
                                                      :end pos
                                                      :external-format
                                                      external-format)))
                             ...
                             (dotimes (i items)
                               (setf (aref buffer (+ start i))
                                     (aref s i))))
			   #-(or (and allegro (version>= 6 0 pre-final 1))
                                 sb-unicode)
			   (dotimes (i tocopy)
			     (setf (aref buffer (+ start i))
...
(defun compute-month (str start)
  ...
  (case (schar str start)
    ...
    (#\F 2 ) ; feb
    (#\J
     (if* (eql (schar str (1+ start)) #\a)
	then 1 ; jan
      elseif (eql (schar str (+ 2 start)) #\l)
	then 7 ; july
	else 6 ; june
	     ))
    (#\M
     (if* (eql (schar str (+ 2 start)) #\r)
	then 3 ; march
	else 5 ; may
	     ))
    (#\N 11) ; nov
...
#-allegro
;; Lock object is in the vhost object.
(defun ensure-stream-lock (stream)
  stream)
