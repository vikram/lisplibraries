...
(defvar *respect-readtable-case* nil
  "If true then symbols a read according to (reatabale-case
  *readtable*), otherwise symbols are read case sensitvly.")
...
(defun symbol-name-respecting-readtable (name)
  (if *respect-readtable-case*
...
(defun compute-tag (coll &optional (package *keyword-package*) ns-to-package)
  ...
  (if* (not ns-to-package)
     then (acl-compat.excl:intern* (symbol-name-respecting-readtable (collector-data coll))
                                   (collector-next coll)
                                   package)
     else
	  (let (new-package (data (collector-data coll)))
	    (if* (and (eq (schar data 0) #\x)
	       ...
	       then ;; putting xmlns: in :none namespace
		    ...
		    (acl-compat.excl:intern* (symbol-name-respecting-readtable (collector-data coll))
                                             (collector-next coll)
                                             package)
	       else
		    (let ((colon-index -1)
			...
			(when (eq (schar data i) #\:)
		      ...
		      (if* (> colon-index -1) then
				...
				(setf new-package (assoc (symbol-name-respecting-readtable string1) ns-to-package :test 'string=))
				(if* new-package
				   then
					...
					(intern (symbol-name-respecting-readtable string2)
                                                (rest new-package))
				   else
					(acl-compat.excl:intern* (symbol-name-respecting-readtable (collector-data coll))
...
(defun unicode-check (p tokenbuf)
  (declare (ignore p tokenbuf) (optimize (speed 3) (safety 1)))
