...
(defun next-token (tokenbuf external-callback attlist-data)
  ...
  (macrolet ((add-to-entity-buf (entity-symbol p-value)
               (declare (ignore entity-symbol))
               (let ((p (gensym)))
                 `(let ((,p ,p-value))
                    (push (make-tokenbuf :cur 0 :max (length ,p) :data ,p)
	   ...
	   (if* (xml-name-char-p ch)
	    ...
	    elseif (eq #\; ch)
	      then (let ((entity-symbol (compute-tag entity)))
		     ...
		     (if* (string= (symbol-name entity-symbol) (symbol-name-respecting-readtable "amp")) then (add-to-coll coll #\&)
		      elseif (string= (symbol-name entity-symbol) (symbol-name-respecting-readtable "lt")) then (add-to-coll coll #\<)
		      elseif (string= (symbol-name entity-symbol) (symbol-name-respecting-readtable "gt")) then (add-to-coll coll #\>)
		      elseif (string= (symbol-name entity-symbol) (symbol-name-respecting-readtable "apos")) then (add-to-coll coll #\')
		      elseif (string= (symbol-name entity-symbol) (symbol-name-respecting-readtable "quot")) then (add-to-coll coll #\")
			else
			     (let (p-value)
	  ...
	  (#.state-readtag4
	   (if* (xml-name-char-p ch) ;; starting char already passed more restrictive test
	      then
		   (add-to-coll coll ch)
	    elseif (eq #\= ch) then
                   (push (subseq (collector-data coll) 0 (collector-next coll)) attrib-name-strings)
		   (setq attrib-name (compute-tag coll *package*
						  (iostruct-ns-to-package tokenbuf)))
		   (clear-coll coll)
		   (let ((name (symbol-name attrib-name)))
		     (when (and (>= (length name) 5)
				(string= name (symbol-name-respecting-readtable "xmlns")
                                         :end1 5))
		       (if* (= (length name) 5)
			  then
                              (setf ns-token :none)
	    ...
	    elseif (xml-space-p ch) then
		   ...
		   (let ((name (symbol-name attrib-name)))
		     (when (and (>= (length name) 5)
				(string= name (symbol-name-respecting-readtable "xmlns") :end1 5))
		       (if* (= (length name) 5)
			  then
			       (setf ns-token :none)
			  else
                         ...
                         (let ((temp-collector (get-collector)))
                           (flet ((make-temp-collector (string)
                             ...
                             (setf attribs-to-return
                                   (loop
                                      for (value nil) on attribs-to-return by #'cddr
                                      for (namestring) on attrib-name-strings 
                                      collect value
                                      collect (compute-tag (make-temp-collector namestring)
                                                           *package*
                                                           (iostruct-ns-to-package tokenbuf))))))
			 )
