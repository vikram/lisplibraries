...
(export '(+threads-p+ make-process
          ...
          make-lock get-lock giveup-lock locked-p with-lock)))
...
(defun make-lock (&key name)
  ...
  #+MCL	       (ccl:make-lock name)
  #+scl        (thread:make-lock name)
  #-threads    (error 'not-implemented :proc (list 'make-lock name)))

;; Under allegro we allow any process to lock and unlock a given 
;; lock by having the lock value t instead of the current process.
;; Look ma, no seatbelts
(defun get-lock (lock)
  "Claims a lock, blocking until the current process can get it."
  #+Allegro    (mp:process-lock lock 1)
...
(defun giveup-lock (lock)
  ...
  #+Allegro    (mp:process-unlock lock 1)
...
(defun locked-p (lock)
  #+Allegro    (and (mp:process-lock-locker lock))
  #-threads    (error 'not-implemented :proc (list 'test-lock lock)))
