...
(defun flatten-strings (args &key separator root)
  (if (null args) root
    (let ((root-val (if root root (car args))))
      (unless root (setf args (cdr args)))
      (when separator
	(setf root-val (concatenate 'string root-val separator)))
      (flatten-strings (cdr args) :root (concatenate 'string root-val (car args)) :separator separator))))
...
(defun run-prog (prog &rest opts &key args (wait t) &allow-other-keys)
  ...
  #+(and allegro (not mswindows)) (apply #'excl:run-shell-command (apply #'vector prog prog args)
					 :wait wait opts)
  #+(and allegro mswindows) (apply #'excl:run-shell-command 
				   (list (flatten-strings (cons prog args) :separator " ") :wait wait))
  #+(and clisp lisp=cl)
