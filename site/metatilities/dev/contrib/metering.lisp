...
(eval-when (compile load eval)
  #+(and :excl (or :allegro-v4.0 :allegro-v4.1 :allegro-v5.0)) 
  (pushnew :modern-allegro *features*))
...
#+(or :allegro-v3.1 :modern-allegro)
...
#+mcl
(defmacro get-cons-overhead ()
  (if (ccl::fixnump (ccl::total-bytes-allocated))
    0 16))
  ...
  (let (required-args)
    ...
    `(lambda (name)
       (let ((inclusive-time 0)
             ...
             (cons-overhead (get-cons-overhead)))
...
;                        (old-nested-calls nested-calls)
                     ...
                     (multiple-value-prog1
                       ...
                       (let ((delta-time (- (get-time) start-time))
                             (delta-cons (- (get-cons) (* 2 cons-overhead) start-cons)))
                         ;; Calls
                         (incf calls)
                         (incf *total-calls*)
...
(defun make-monitoring-encapsulation (min-args optionals-p)
  (let (required-args)
    (dotimes (i min-args) (push (gensym) required-args))
    `(lambda (name)
       (let ((inclusive-cons 0)
             (exclusive-cons 0)
             (calls 0)
             (nested-calls 0)
             (old-definition (place-function name))
             (cons-overhead (get-cons-overhead)))
         (declare (type consing-type inclusive-cons)
                  (type consing-type exclusive-cons)
                  (fixnum calls)
                  (fixnum nested-calls))
         (pushnew name *monitored-functions*)
         ...
         (setf (place-function name)
               #'(lambda (,@required-args
                          ,@(when optionals-p `(&rest optional-args)))
                   (let ((prev-total-cons *total-cons*)
                         (start-cons (get-cons)))
                     (declare (type consing-type start-cons)
                              (type consing-type prev-total-cons))
                     (multiple-value-prog1
                       ,(if optionals-p
                          `(apply old-definition 
                                  ,@required-args optional-args)
                          `(funcall old-definition ,@required-args))
                       (let* ((delta-cons (- (get-cons) (* 2 cons-overhead) start-cons)))
                         ;; Calls
                         (incf calls)
                         (incf *total-calls*)
...
#|
(defun foo ()
  (status (execution-engine (current-world-state))))
...
Function  Time  Cons  Calls  Sec/Call    Call       Time     Cons
-------------------------------------------------------------------
FOO       1.00  1.00   1000  0.000004      32   0.004200     32000
-------------------------------------------------------------------
TOTAL:    1.00  1.00   1000                   0.004200   32000


(mon:monitor foo status execution-engine current-world-state)

                                                    Cons
                     %     %                        Per        Total    Total
Function             Time  Cons  Calls  Sec/Call    Call       Time     Cons
------------------------------------------------------------------------------
FOO                  0.70  0.65   1000  0.000025     176   0.025200    176000
CURRENT-WORLD-STATE  0.15  0.12   1000  0.000005      32   0.005200     32000
STATUS               0.09  0.12   1000  0.000003      32   0.003200     32000
EXECUTION-ENGINE     0.06  0.12   1000  0.000002      32   0.002200     32000
------------------------------------------------------------------------------
TOTAL:               1.00  1.00   4000                   0.035800  272000


(defun foo ()
  (execution-engine (current-world-state)))

                                                    Cons
                     %     %                        Per        Total    Total
Function             Time  Cons  Calls  Sec/Call    Call       Time     Cons
------------------------------------------------------------------------------
FOO                  0.79  0.67   1000  0.000009     128   0.009200    128000
CURRENT-WORLD-STATE  0.19  0.17   1000  0.000002      32   0.002200     32000
EXECUTION-ENGINE     0.02  0.17   1000  0.000000      32   0.000200     32000
------------------------------------------------------------------------------
TOTAL:               1.00  1.00   3000                   0.011600  192000
...
Function             Time  Cons  Calls  Sec/Call    Call       Time     Cons
------------------------------------------------------------------------------
CURRENT-WORLD-STATE  0.65  0.29   1000  0.000002      32   0.002200     32000
FOO                  0.35  0.71   1000  0.000001      80   0.001200     80000
------------------------------------------------------------------------------
TOTAL:               1.00  1.00   2000                   0.003400  112000



(time (loop repeat 1000 do (foo)))
(mon:monitor foo)
(mon:monitor foo status execution-engine current-world-state)
(loop repeat 1000 do (foo))
(foo)
(mon::report-monitoring :all :exclusive 0.00)
(mon:unmonitor)

(mon::report-monitoring :all :exclusive 0.01)
                                                             Cons
 
|#
#+Ignore
;; Gary King 2004-02-18: this monitors only consing ?? untested
(defun make-monitoring-encapsulation (min-args optionals-p)
  (let (required-args)
    (dotimes (i min-args) (push (gensym) required-args))
    `(lambda (name)
                     ...
                     (multiple-value-prog1
                       ,(if optionals-p
                          `(apply old-definition 
                                  ,@required-args optional-args)
                          `(funcall old-definition ,@required-args))
                       (let ((delta-cons (- (get-cons) start-cons)))
                         ;; Calls
                         (incf calls)
...
(defmacro with-package-monitoring ((&rest packages)
                                   (&rest options &key (report? t) (nested :exclusive) 
                                          (threshold 0.01) (key :percent-time)
                                          (excluded-functions nil) (reset? t)
                                          &allow-other-keys)
                                   &body body)
  "Monitor the specified packages during the execution of the body."
  (loop for arg in '(:report? :nested :threshold :key 
                     :excluded-functions :reset?) do
        (remf options arg))
  `(unwind-protect 
     (progn
       (mapc (lambda (package)
               (monitor-all (find-package package)))
             ',packages)
       ,@(when excluded-functions
           `((dolist (symbol ,(if (consp excluded-functions) 
                              excluded-functions 
                              (list excluded-functions)))
             (monitoring-unencapsulate symbol))))
       ,@(when reset?
           `((reset-all-monitoring)))
       ,@body
       ,@(when report?
...
(defun display-monitoring-results (&optional (threshold 0.01) (key :percent-time)
  ...
  (let ((max-length 8)                  ; Function header size
    ...
    (dolist (result *monitor-results*)
      (let* ((colon-pos (position #\: (m-info-name result)))
             (colon-count (count #\: (m-info-name result))))
        (when colon-pos
          (setf (m-info-name result)
                (format nil "~A~:[:~;::~]~A"
                        (ccl::shortest-package-nickname (subseq (m-info-name result) 0 colon-pos))
                        (= colon-count 2)
                        (subseq (m-info-name result) (+ colon-pos colon-count)))))))
