...
(defun ensure-scanner (expr &rest options)
  (cond ((symbolp expr)
	 (apply #'cl-ppcre:create-scanner (cons (symbol-name expr) options)))
	((stringp expr)
	 (apply #'cl-ppcre:create-scanner (cons expr options)))
	(t expr)))
...
(defun-exported grep (regex data)
  (let* ((scanner (ensure-scanner regex :case-insensitive-mode t :multi-line-mode t)))
    (on-trees (nconc left right)
	      (if (and (stringp it) (cl-ppcre:scan scanner it))
		  (mklist it)
		nil)
	      data)))
...
;;		     nil)
;;		    ((stringp data)
;;		     (if (cl-ppcre:scan regex-opt data)
;;			 (mklist data)))
;;		    ((consp data)
