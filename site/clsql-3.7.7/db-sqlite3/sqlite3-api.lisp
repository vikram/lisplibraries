...
;;;; Created:  Oct 2004
;;;;
;;;; $Id: sqlite3-api.lisp 10191 2004-12-07 06:11:55Z kevin $
;;;;
  ...
  (:use #:common-lisp #:uffi)
    (:export
	   ...
	   ;;; API functions.
           #:sqlite3-open
	   #:sqlite3-close
	   #:sqlite3-enable-load-extensions
...
(def-sqlite3-function
    ("sqlite3_enable_load_extension" %enable-extensions)
    ((db sqlite3-db)
     (onoff :int))
  :returning :int)
...
;	      (sqlite3-enable-load-extensions db)
	      db))))))
...
(defun sqlite3-enable-load-extensions (db)
  (declare (type sqlite3-db-type db))
  (let ((result (%enable-extensions db 1)))
    (if (/= result 0)
	(signal-sqlite3-error result)
	t)))
