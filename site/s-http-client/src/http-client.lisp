...
;;;;
;;;; Copyright (C) 2005,2006 Sven Van Caekenberghe, Beta Nine BVBA.
;;;;
...
(defun response-read-chunked-body (stream buffer)
...
(defun read-response (stream buffer &optional (body-expected t))
    ...
    (when (and body-expected (not (= response-code 204)))
      (let* ((content-length-header (cdr (find :content-length response-headers :key #'car)))
        ...
        (setf response-body (if chunked-p
                                (response-read-chunked-body stream buffer)
                              (if content-length
                                  (response-read-body stream content-length)
                                (response-read-undefined-body stream buffer))))))
...
(defun do-one-request-response (connection 
  ...
  (let ((body-expected (if (eql method :head) nil t)))
    (values-list `(,@(multiple-value-list (read-response connection buffer body-expected)) ,uri))))
...
(defun do-http-request-internal (scheme host port state 
                                 method uri content content-type basic-authorization headers
