  ...
  (setf (lambda-list-mandatory-parameters self)
        (loop
           ...
           :collect (if (consp (car current))
                        ...
                        (parse-parameter
                         (make-instance 'specialized-parameter)
                         (pop current)))))
  current)
...
TODO:   See what happens for (&BODY) or (&REST).
"
...
"
  (append
   (when (lambda-list-body-parameter self)
     (list '&whole (parameter-specifier (lambda-list-whole-parameter self))))
   (when (lambda-list-body-parameter self)
     (list '&environment (parameter-specifier
   ...
   (when (lambda-list-optional-parameters self)
     (cons '&optional (mapcar (function parameter-specifier)
                              (lambda-list-optional-parameters self))))
   (when (lambda-list-body-parameter-p self)
