...
(DEFPACKAGE "COM.INFORMATIMAGO.COMMON-LISP.SOURCE-TEXT"
  (:NICKNAMES "SOURCE-TEXT")
  (:USE "COMMON-LISP"
...
(defun get-line-and-column (line-positions pos)
  (multiple-value-bind (foundp index order)
      (dichotomy line-positions pos (lambda (a b) (cond ((< a b) -1)
                                                   ((> a b)  1)
                                                   (t        0))))
...
"
  (values t (building-source-object *stream*  *start* 'source-token
                                    :token  (reader::token-text   token)
                                    :traits (reader::token-traits token))))
   ...
   :value (flet ((error-eof ()
            ...
            (loop
               ...
               :do (cond
                     ...
                     ((= (reader::character-syntax
                          (reader::character-description rst ch))
                         reader::+cs-single-escape+)
...
"
  ...
  (building-reader-dispatch-macro-source-object
   stream arg sub-char
   'source-bit-vector
   :elements (reader::reader-dispatch-macro-bit-vector stream arg sub-char)))
...
(defun source-reader-dispatch-macro-BINARY            (stream arg sub-char)
  ...
  (building-reader-dispatch-macro-source-object
   ...
   :base 2.
   :specificp t
   :value (com.informatimago.common-lisp.reader::read-rational-in-base
...
(defun source-reader-dispatch-macro-RADIX             (stream arg sub-char)
  ...
  (building-reader-dispatch-macro-source-object
   ...
   :specificp nil
   :value (if arg
              (com.informatimago.common-lisp.reader::read-rational-in-base
...
(defun source-reader-dispatch-macro-COMPLEX           (stream arg sub-char)
  ...
  (building-reader-dispatch-macro-source-object
   ...
   :subform (let ((c (source-read stream t nil t)))
              (if (and (com.informatimago.common-lisp.reader::proper-list-p c)
                       (= 2 (length c))
...
(defun source-read (&optional input-stream
  ...
  (let ((*read-suppress* nil)           ; we do want the source! 
    ...
    (let ((*start* (file-position input-stream)))
      ...
      (reader::read-1  input-stream eof-error-p eof-value
...
#- (and)
(with-input-from-string (src "  ")
  (com.informatimago.common-lisp.READER::READ-0/1 src nil :eof nil NIL NIL T))
...
(defparameter *some*
  (with-open-file (src "source-text.lisp")
    (loop
       :for item = (com.informatimago.common-lisp.source-text:source-read
                    src nil src)
       :until (eq item src)
       :collect item
       :until (typep item 'com.informatimago.common-lisp.source-text:source-lexical-error)
