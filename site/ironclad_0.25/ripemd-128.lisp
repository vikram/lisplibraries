...
(defclass ripemd-128 ()
  ((regs :reader registers :type ripemd-128-regs
         :initform (initial-ripemd-128-regs))
   (amount :accessor amount-processed :type (unsigned-byte 64) :initform 0)
   (block :reader ripemd-128-block :type (simple-array (unsigned-byte 32) (16))
...
(defmethod reinitialize-instance ((state ripemd-128) &rest initargs)
  ...
  (replace (the ripemd-128-regs (registers state))
           +pristine-ripemd-128-registers+)
  (setf (amount-processed state) 0
        (buffer-index state) 0)
  state)

(defmethod copy-digest ((state ripemd-128))
  (let ((copy (make-instance 'ripemd-128)))
...
FINALIZE-RIPEMD-128-STATE results in unspecified behaviour."
  (let ((old-regs (registers state))
    ...
    (declare (type ripemd-128-regs old-regs)
             (type (integer 0 63) buffer-index)
             (type (simple-array (unsigned-byte 32) (16)) block)
...
(defdigest ripemd-128 :digest-length 16)
