...
(declaim (type (simple-array (unsigned-byte 32) (512)) tiger-t1 tiger-t2 tiger-t3 tiger-t4))
(defparameter tiger-t1
  (make-array 512 :element-type '(unsigned-byte 32)
              :initial-contents
...
(defclass tiger ()
  ((regs :reader registers :type tiger-regs :initform (initial-tiger-regs))
   (amount :accessor amount-processed :type (unsigned-byte 64) :initform 0)
   (block :reader tiger-block :type (simple-array (unsigned-byte 32) (16))
...
(defmethod reinitialize-instance ((state tiger) &rest initargs)
  ...
  (replace (the tiger-regs (registers state)) +pristine-tiger-registers+)
  (setf (amount-processed state) 0
        (buffer-index state) 0)
  state)

(defmethod copy-digest ((state tiger))
  (let ((copy (make-instance 'tiger)))
...
FINALIZE-TIGER-STATE results in unspecified behaviour."
  (let ((old-regs (registers state))
        ...
        (regs (make-array 6 :element-type '(unsigned-byte 32)))
        (buffer (make-array 64 :element-type '(unsigned-byte 8))))
    (declare (type tiger-regs old-regs regs)
             (type (integer 0 63) buffer-index)
             (type (simple-array (unsigned-byte 32) (16)) block)
             (type (simple-array (unsigned-byte 8) (*)) old-buffer buffer)
...
(defdigest tiger :digest-length 24)
