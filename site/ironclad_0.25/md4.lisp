...
(defclass md4 ()
  ((regs :reader registers :type md4-regs :initform (initial-md4-regs))
   (amount :accessor amount-processed :type (unsigned-byte 64) :initform 0)
   (block :reader md4-block :type (simple-array (unsigned-byte 32) (16))
...
(defmethod reinitialize-instance ((state md4) &rest initargs)
  ...
  (replace (the md4-regs (registers state)) +pristine-md4-registers+)
  (setf (amount-processed state) 0
        (buffer-index state) 0)
  state)

(defmethod copy-digest ((state md4))
  (let ((copy (make-instance 'md4)))
...
FINALIZE-MD4-STATE results in unspecified behaviour."
  (let ((old-regs (registers state))
    ...
    (declare (type md4-regs old-regs regs)
             (type (integer 0 63) buffer-index)
             (type (simple-array (unsigned-byte 32) (16)) block)
             (type (simple-array (unsigned-byte 8) (*)) old-buffer buffer)
             (dynamic-extent buffer regs))
    ;; Copy over.
...
(defdigest md4 :digest-length 16)
