...
(defclass ripemd-160 ()
  ((regs :reader registers :type ripemd-160-regs
         :initform (initial-ripemd-160-regs))
   (amount :accessor amount-processed :type (unsigned-byte 64) :initform 0)
   (block :reader ripemd-160-block :type (simple-array (unsigned-byte 32) (16))
...
(defmethod reinitialize-instance ((state ripemd-160) &rest initargs)
  ...
  (replace (the ripemd-160-regs (registers state))
           +pristine-ripemd-160-registers+)
  (setf (amount-processed state) 0
        (buffer-index state) 0)
  state)

(defmethod copy-digest ((state ripemd-160))
  (let ((copy (make-instance 'ripemd-160)))
...
`finalize-ripemd-160-state' results in unspecified behaviour."
  (let ((old-regs (registers state))
    ...
    (declare (type ripemd-160-regs old-regs)
             (type (integer 0 63) buffer-index)
             (type (simple-array (unsigned-byte 32) (16)) block)
...
(defdigest ripemd-160 :digest-length 20)
