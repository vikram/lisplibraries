...
(defclass md5 ()
  ((regs :reader registers :type md5-regs :initform (initial-md5-regs))
   (amount :accessor amount-processed :type (unsigned-byte 64) :initform 0)
   (block :reader md5-block :type (simple-array (unsigned-byte 32) (16))
...
(defmethod reinitialize-instance ((state md5) &rest initargs)
  ...
  (replace (the md5-regs (registers state)) +pristine-md5-registers+)
  (setf (amount-processed state) 0
        (buffer-index state) 0)
  state)

(defmethod copy-digest ((state md5))
  (let ((copy (make-instance 'md5)))
...
FINALIZE-MD5-STATE results in unspecified behaviour."
  (let ((old-regs (registers state))
    ...
    (declare (type md5-regs old-regs regs)
             (type (integer 0 63) buffer-index)
             (type (simple-array (unsigned-byte 32) (16)) block)
...
(defdigest md5 :digest-length 16)
