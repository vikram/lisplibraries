  ...
  (let ((test-function (test-function self)))
	...
	(let ((newj2len (make-hash-table :test test-function)))
	  ...
	  (loop while (and (> best-i alo)
			   (> best-j blo)
			   (not (funcall b-junk-function (elt b (- best-j 1))))
	  ...
	  (loop while (and (< (+ best-i best-size) ahi)
			   ...
			   (not (funcall b-junk-function (elt b (+ best-j best-size))))
...
(defun unified-diff (stream a b &key from-file to-file from-file-date to-file-date (n 3)
		     (test-function #'eql))
  (let ((started NIL)
...
(defun context-diff (stream a b &key from-file to-file from-file-date to-file-date (n 3)
		     (test-function #'eql))
